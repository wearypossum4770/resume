...
function calculatePresentValueSingleSum(array) {
  const obj = {
    number_of_periods: array[2],
    interest_rate: array[1],
    annual_payment: array[0],
  };
  return parseFloat(presentValueSingleSum(obj)) * obj.annual_payment;
}
function calculateFutureValueOrdinaryAnnuity(array) {
  const obj = {
    number_of_periods: array[2],
    interest_rate: array[1],
    annual_payment: array[0],
  };
  return parseFloat(futureValueOrdinaryAnnuity(obj)) * obj.annual_payment;
}
function calculatePresentValueOrdinaryAnnuity(array) {
  const obj = {
    number_of_periods: array[2],
    interest_rate: array[1],
    annual_payment: array[0],
  };
  return parseFloat(presentValueOrdinaryAnnuity(obj)) * obj.annual_payment;
}
function calculatePresentValueAnnuityDue(array) {
  const obj = {
    number_of_periods: array[2],
    interest_rate: array[1],
    annual_payment: array[0],
...
const futureValueSingleSum = ({ interest_rate, number_of_periods }) =>
  parseFloat(Math.pow(1 + interestRate(interest_rate), number_of_periods));
